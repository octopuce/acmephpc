...
/**
 * Acme SSL functions class implemented with PhpSecLib
 * @author benjamin
 */
class SslPhpseclib implements SslInterface {
    ...
    function genRsa() {
        $rsa = new RSA();
        $privKey = $rsa->createKey(4096); // TODO handle timeout? 
        if (isset($privKey["partialkey"])) {
            unset($privKey["partialkey"]);
        }
        return $privKey; // hash with privatekey and publickey  
    }
